 ...
 */
let $_ = (get: string, set: string | number = '') => {
  switch (get) {
 ...
 * date: Return date
 */ 
let date = (format = '', type: any = '') => {
  // New date object
  var date = new Date()

  // return Alwayse two digits
  const digit = function (i: any) {
    if (i < 10) i = '0' + i
    return i
  }
  ...
  // i: Minutes
  const i: number = digit(date.getMinutes())

  // s: Seconds
  const s: number = digit(date.getSeconds())

  // ms: Seconds
  const ms: number = date.getMilliseconds()

  // t: Timezone
  const t: number = date.getTimezoneOffset()
...
// Attr: Return element attribute
let attr = (attrib: any, el: string, set?: any) => {
  let x: any = select(el)
...
// classListToggle: 
let classListToggle = (el: string, toggles: string[]) => {
  let x: any = select(el);
  
  toggles.forEach(token => {
    x.classList.toggle(token)
  });
}
...
let select = (el: string, all: boolean = false) => {
  if (isset(el)) {
    try {
      if (typeof (el) == 'string' && el != null) el = el.trim()
...
let selectId = (el: string) => {
  try {
    if (typeof (el) == 'string' && el != null) el = el.trim()
    return document.getElementById(el)
  } catch (selectId_ERR: any) {
...
let echo = (text: any, el?: any, all = false) => {
  if (isset(el)) {
    if (all) {
      let selection: any = select(el, all)
          selection.forEach((el: { innerHTML: string; }) => {
            el.innerHTML = `${text}`
          })
    } else {
      try {
...
let textContent = (el: any, value?: string) => {
  if (isset(value)) {
...
let htmlContent = (el: any, value?: string) => {
  if (isset(value)) {
    if (isset(el)) {
      el.innerHTML = `${value}`
    } else {
...
let display = (el: any, displayValue: string = 'block') => {
  const d: any = select(el);
...
let hide = (el: any) => {
  const x: string[] = ['block', 'inline-block', 'flex', 'inline-flex']
  const d: any = select(el)
        d.classList.add('hidden')
 ...
 */
// On: Events listner
let on = (event: string, listener: any) => {
  event = event.trim()
  document.addEventListener(event, listener)
}

// Onclick: Click listner
let onClick = (el: any, listener: any) => {
  const d: any = selectId(el)
  return d.onclick = listener
}

// Ondlclick: Double click listner
let onDblClick = (el: any, listener: any) => {
  const d: any = selectId(el)
...
/**
 * Validation
 * Check for valid listner
 */
// Isset: Return true for no-empty Object/var
let isset = (key: any): boolean => {
...
let setCookie = ( cookieName: any, cookieValue: any, cookieExpiry?: any ) => {
  try {
...
let unsetCookie = (cookieName: any, cookieExpiry?: any ) => {
  try {
    if (isset(cookieExpiry)) {
